...
def applyToCycleVoting(request):
    votaciones = Votacion.objects.filter(idFacultad=Estudiante.objects.get(id=request.user.id).idFacultad_id,semestre=Estudiante.objects.get(id=request.user.id).semestre)
